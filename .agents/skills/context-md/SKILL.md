...
Most repos have a single context with a `CONTEXT.md` at the root.
...
When an `AGENTS.md` is present and does not reference the root `CONTEXT.md` or `CONTEXT-MAP.md`, offer to add a reference. This ensures other agents with a different skill set can benefit from the registered context concepts.
